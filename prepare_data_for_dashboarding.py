...
years = split_data_by_year(fullMerged)
...
years = split_and_merge(lutron, years, False)

# Concatenating each year of data together
dashboardData = pd.concat([years[0], years[1], years[2], years[3], years[4]])

#Filtering for the most recent month of data
dashboardDataMonth = dashboardData[dashboardData["Date"] == dashboardData["Date"].max()]

# Merging in the population and diversity growth data with the most recent month of data
dashboardDataMonth = merge_in_growth(dashboardDataMonth)

# Converting the dates to a better format
dashboardData['Date'] = dashboardData['Date'].astype(str)
dashboardData['Date'] = pd.to_datetime(dashboardData['Date'], format='%Y%m', errors='coerce')
dashboardDataMonth['Date'] = dashboardDataMonth['Date'].astype(str)
dashboardDataMonth['Date'] = pd.to_datetime(dashboardDataMonth['Date'], format='%Y%m', errors='coerce')

# Saving the final merged data set to CleanedDataCSVs
dashboardData.to_csv("CleanedDataCSVs/cleaned_dashboard_data.csv", sep=',', index=False, encoding='utf-8')
dashboardDataMonth.to_csv("CleanedDataCSVs/cleaned_dashboard_month_data.csv", sep=',', index=False, encoding='utf-8')